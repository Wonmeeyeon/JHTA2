...
public class FileDownloadView extends AbstractView{
	...
	// 응답컨텐츠의 타입을 application/octet-stream 으로 설정한다.
	// application/octet-stream은 알려지지 않은 파일의 파일은 지정할 때 이 타입을 사용한다.
	// response.setHeader(name, value)는 응답메세지의 헤더정보를 설정할 때 사용한다.
	// * 응답컨텐츠타입, 응답컨텐츠 길이, 응답 컨텐츠와 관련된 부가정보 설정
	// Content-Disposition으로 다운로드되는 파일의 이름을 브라우저에게 제공할 수 있다.
	// value값의 attachment는 항상 파일을 다운로드하라는 것이다.
	// (브라우저에 표시하지 말고)
	// URLEncoder.encode(file, "utf-8"))는 파일이름에 한글이 포함되있을 경우
	// utf-8 방식으로 변환해서 파일명을 응답헤더에 포함시킨다.
