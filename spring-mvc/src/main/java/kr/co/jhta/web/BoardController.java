 ...
 *  	@RequestMapping
 *  		- 요청 URL와 컨트롤러 혹은 요청 URL과 요청 핸들러 메소드를 매핑 시킨다.
 *  		- 클래스와 메소드에 각각 부착할 수 있다.
 *  		- 클래스에 부착할 경우, 해당 클래스에 정의된 모든 요청 핸들러 메소드의 매핑 URI에 접두사로 추가된다.
 ...
 *  						// 아이디, 비밀번호를 POST 요청청방식을 ㅗ서버로 보냈을 때 요청 처리
 *  				
 *  			consumes
 ...
 *  				* 위 세가지 모두 요청파라미터명이 uerId인 요청파라미터값을 찾아서
 *  				  매개변수 id에 전달한다.
 ...
 *  				*** 주의 사항) 반드시 지정한 이름의 요청파라미터가 존재해야한다.
 *  
 *  				
 *  			name
 *  				요청파라미터 명을 지정한다.
 ...
 *  				required=false로 지정하지 않은 이상 무조건 요청파라미터가 존재해야한다.
 *  				required=false로 지정하는 경우 거의 대부분 defaultValue와 같이 사용된다.
 *  				
 *  			defaultValue
 *  				요청 파라미터 값이 비어있을 때 기본 값을 지정한다.
 *  				required=false로 지정된 경우 defaultValue를 사용해서 기본값을 지정할 수 있다.
 *  				@RequstParam(name="pageNo", required=false, defaultValue="1") int pageNo
...
public class BoardController {
	...
	// *<form />태그의 모든 입력필드값을 담는 저장하는 폼 커맨드 객체를 요청핸들러 메소드의
	// 	매개변수로 선언하면, HandlerAdapter가 요청파라미터값을 조회해서
	//  폼 커맨드객ㅊ체에 자동으로 저장한 후, 요청 핸들러 메소드 실행시 전달해준다.
	// * <form />태그의 입력필드갯수가 많을 때 반복적인 코드를 효과적으로 줄일수 있다.
	public String addBoard(Boardform boardform, User user)  throws Exception{
		...
		// (필드명은 동일하지만, 타입이 다른 필드가 존재하지 않아야한다.)
		...
		// MultipartFile 
		// * 첨부파일 업로드를 지원하는 객체다.
		// * <form />태그 enctype="multipart/form-data"이고, 첨부파일 필드가 있는 경우
		//  xxxForm객체에 첨부파일 입력필드와 동일한 이름의 필드가 있으면,
		//  MultipartFile는 결코 null이 아니다.
		//  (<input type="file"/> 입력필드에서 첨부파일은 설택하지 않아도
		...
		// 6. FileCoypUtils.copy(소스, 대상)를 사용해서 서버의 임시디렉토리에 저장된
	...
	public String likesBoard(@RequestParam("no") long boardNo, 
		 ...
		 boardService.inceraseBoardLikes(boardNo);
