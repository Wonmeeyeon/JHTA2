 ...
 * 		- HandlerMethodArgumentResolver를 구현해서 사용자지정의 ArgumentResolver를
 * 		    정의하면, 요청핸들러 메소드 실행시 필요한 객체를 ArgumentResolver를 통해서 
 * 		  제공받을 수 있다.
 * 		- 예)
 *  		* 로그인한 사용자정보가 필요한 요청핸들러 메소드에서 세션에 저장된 로그인 사용자 
 *  		  정보가  저장된 객체를 제공하는 ArgumentResolver를 정의할 수 있다.
 */

public class UserArgumentResolver implements HandlerMethodArgumentResolver{

	//  *요청핸들러 메소드의 매개변수가 사용자가 지정한 타입의 매개변수와 일치하면 true를 반환하는 메소드다.
	...
	//   매개변수로 전달된다.
	// * UserArgumentResilver는 요청핸들러 메소드의 매개변수 타입이 User인
